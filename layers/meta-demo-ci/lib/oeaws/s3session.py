...
class S3Session(object):
    ...
    def makeclient(self):
        ...
        bb.info(1, "Using AWS profile: %s" % provider._profile_name)
    ...
    def download(self, Bucket, Key, Filename, quiet=True):
        ...
        for attempt in range(10):
            try:
                bb.info(2, "%s/%s: attempt %d" % (Bucket, Key, attempt))
                ...
                if quiet and err['Code'] == "404":
                    bb.info(2, "not found: {}/{}".format(Bucket, Key))
                else:
                    bb.warn("{}/{}: {} {}".format(Bucket, Key, err['Code'], err['Message']))
                return False
            except OSError as e:
                ...
                return False
            bb.info(1, "{}/{}: success".format(Bucket, Key))
            return True
